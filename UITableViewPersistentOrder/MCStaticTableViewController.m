...
#import "MCStaticTableViewController.h"

@interface MCStaticTableViewController ()

@end

@implementation MCStaticTableViewController {
...
- (void)persistArray
{
    [def setValue:array forKey:@"array"];
    [def synchronize];
}
...
- (void)viewDidLoad
{
    ...
    array = [def valueForKey:@"array"];
    if (!array) {
        array = [@[@"A", @"B", @"C", @"D"] mutableCopy];
        [self persistArray];
    }
    self.editing = YES;
}
